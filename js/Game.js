...
Pacman.Game.prototype = {
 ...
 followRobot: function() //  Simple pattern for my enemy to follow me.
    ...
    TextNewLevel: function() //Function shows the new level text
    {   
         if(game.time.now>TextTime){
            stateText2.text=" YOU WON \n Click to continue \n to Level 2";
            stateText2.visible = true;
            TextTime=game.time.now+10000;}
    },
    ...
    update: function () 
    {
    	...
    	{						 //  It has a problem because when i teleport to the new position i cant move my player
          ...
          if (score==500){ //If score=500 go to next level
            this.TextNewLevel();
            game.input.onTap.addOnce(function(){this.state.start('Level2')},this);
